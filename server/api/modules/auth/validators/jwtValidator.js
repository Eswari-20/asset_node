...
exports.generateNewToken = (token, secretOrPrivateKey, ignoreExp) => {
  ...
  var verifyOptions = { audience: 'http://dreamstep.com', subject: payload.mrcht_usr_id + '_user', ignoreExpiration: ignoreExp, algorithm: ["HS256"] };

  try {
    //verified = jwt.verify(token, secretOrPrivateKey);
    // verified = jwt.verify(token, 'dreamstepsunil', { algorithm: 'HS256'});
    verified = jwt.verify(token, secretOrPrivateKey, { algorithm: 'HS256' });
