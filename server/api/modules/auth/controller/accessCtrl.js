    ...
    let token = req.headers['x-access-token'];
            ...
            */
            ...
            if (validateToken.isExpired(token, validSeconds, user)) {
                const accessToken = validateToken.generateNewToken(token, 'dreamstepsunil', ignoreExp);
                ...
                //console.log("in else")
                var verified;
                var verifyOptions = {algorithm: 'HS256', keyid: '1', expiresIn: '3h'};
                //console.log(verifyOptions)
                //console.log("check algorithn",jwt.verify(token, 'BssGlitsUser'));
                //console.log(jwt.verify(token, 'BssGlitsUser', { algorithm: 'HS256'}));
                try {
                    verified = jwt.verify(token, 'glitsuserRamesh', {algorithm: 'HS256', keyid: '1', expiresIn: '3h'});
                }
                catch (err) {
                    verified = null;
                }
