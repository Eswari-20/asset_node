// Standard Inclusions
// var appRoot= require('/glits/web/code/iDoseMate/node');
...
function conn_execSQL(qryEle, Qry, ConPool, cntxtDtls, userDtls, fnm, callback) {
      ...
      //console.log(Qry);
      // ConPool.getConnection(function (err, connection) {    // get connection from Connection Pool 
      //       // if (err) { console.log("356",err); log.db.conError(cntxtDtls, Qry, err.code, err.fatal); callback(err, null); return err; }
	...
	// 	// 			//console.log(rows)
      ...
      //       //       if (err) { console.log(err); log.db.qryError("ERROR", cntxtDtls, Qry, err.code, err.sqlMessage); callback(err, null); return; } // Handle Query Errors         
...
exports.execParamsQuery = function (ConPool, Qry, params, cntxtDtls, callback) {
    if (!Qry) {
        console.log('No Query to execute');
        if (typeof callback === 'function') callback(new Error('No Query to execute'), null);
        return;
    }

    Qry = Qry.trim();
    const multiQrs = sqlParser.multiQrs(Qry);

    const len = multiQrs.length;
    let completedQueries = 0;
    let qry_rslts = [];

    function handleQueryCompletion(err, results) {
        if (err) {
        ...
        if (completedQueries === len) {
            if (typeof callback === 'function') callback(null, len > 1 ? qry_rslts : qry_rslts[0]);
        }
    }

    if (typeof callback === 'function') {
        multiQrs.forEach((query, index) => {
            const queryParams = params[index] || [];
            executeParamsEachQuery(query, queryParams, ConPool, cntxtDtls, handleQueryCompletion);
        });
    } else {
        return new Promise((resolve, reject) => {
            multiQrs.forEach((query, index) => {
                const queryParams = params[index] || [];
                executeParamsEachQuery(query, queryParams, ConPool, cntxtDtls, (err, results) => {
                    if (err) {
                        reject(err);
                    } else {
                        handleQueryCompletion(null, results);
                    }
                });
            });
        });
...
function executeParamsEachQuery(query, queryParams, ConPool, cntxtDtls, callback) {
    if (!query) {
        if (typeof callback === 'function') callback(new Error('Invalid query'), []);
        return;
    }
    ...
    ConPool.getConnection(function (err, connection) {
        if (err) {
            ...
            log.db.conError(cntxtDtls, query, err.code, err.fatal);
            if (typeof callback === 'function') callback(err, null);
            return;
        }
        ...
        connection.query(query, queryParams, function (err, rows) {
            ...
            if (err) {
                ...
                log.db.qryError("ERROR", cntxtDtls, query, err.code, err.sqlMessage);
                if (typeof callback === 'function') callback(err, null);
                return;
            }
